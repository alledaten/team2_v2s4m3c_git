...
public class ReviewProc implements ReviewProcInter{
  ...
  public List<Review_Member_ProductVO> list_all(HashMap<String, Object> map) {
    ...
    */
    ...
    List<Review_Member_ProductVO> list = this.reviewDAO.list_all(map);
    return list;
  }
  ...
  public int search_count_all(HashMap<String, Object> hashMap) {
    int count = reviewDAO.search_count_all(hashMap);
    return count;
  }
...
//    str.append("현재 페이지: " + nowPage + " / " + totalPage + "  "); 
  ...
  public String pagingBox_all(String listFile, int search_count_all, int nowPage, String review_word) { 
    int totalPage = (int)(Math.ceil((double)search_count_all/Review.RECORD_PER_PAGE)); // 전체 페이지  
...
//    str.append("현재 페이지: " + nowPage + " / " + totalPage + "  "); 
    ...
    int _nowPage = (nowGrp-1) * Review.PAGE_PER_BLOCK;  
    if (nowGrp >= 2){ 
      str.append("<span class='span_box_1'><A href='"+listFile+"?&review_word="+review_word+"&nowPage="+_nowPage+"'>이전</A></span>"); 
    } 
    ...
    for(int i=startPage; i<=endPage; i++){ 
      ...
      if (nowPage == i){ // 페이지가 현재페이지와 같다면 CSS 강조(차별을 둠)
        ...
        // 현재 페이지가 아닌 페이지는 이동이 가능하도록 링크를 설정
        str.append("<span class='span_box_1'><A href='"+listFile+"?review_word="+review_word+"&nowPage="+i+"'>"+i+"</A></span>");   
      } 
    } 
    ...
    _nowPage = (nowGrp * Review.PAGE_PER_BLOCK)+1;  
    if (nowGrp < totalGrp){ 
      str.append("<span class='span_box_1'><A href='"+listFile+"?&review_word="+review_word+"&nowPage="+_nowPage+"'>다음</A></span>"); 
    } 
    str.append("</DIV>"); 
